...
void mode_A(string s, string m) {
    ...
    if (parameter[0] == "add") {
        ...
        for (Coat& coat : controller.myVectors) {
            in << coat.getName() << " " << coat.getSize() << " " << coat.getPrice() << " " << coat.getPhoto() << "\n";
        }
...
int main() {
    ...
    /*for (i = 2; i < number_parameters; i++) {
        parameter_text[i].erase(parameter_text[i].begin());
    }*/

    if (parameter_text[0] == "fileLocation") {
        cout << number_parameters;
        for (i = 1; i <= number_parameters; i++) {
            cout << parameter_text[i] << endl;
